import { fetchLinkedInProfileData } from '../utils/linkedinScraper.js';
import Lead from '../models/Lead.js';

export const scrapeLinkedIn = async (req, res) => {
  try {
    const { profileUrl, filters } = req.body;
    if (!profileUrl) return res.status(400).json({ error: 'LinkedIn URL required' });

    const scrapedLeads = await fetchLinkedInProfileData(profileUrl);
    ...
    res.json({ message: `✅ Saved ${saved.length} LinkedIn leads`, leads: saved });
  } catch (error) {
